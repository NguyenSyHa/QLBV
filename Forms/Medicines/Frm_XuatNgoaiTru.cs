...
{
    public partial class Frm_XuatNgoaiTru : DevExpress.XtraEditors.XtraUserControl
    {
        private QLBVEntities _dataContext; 
        private readonly MedicinesProvider _medicinesProvider;
        public Frm_XuatNgoaiTru()
        {
            InitializeComponent();
        ...
        List<datasoureBN> dt = new List<datasoureBN>();
        int _makp = DungChung.Bien.MaKP;// makp tìm kiếm
        int _maKhoXuat = 0;
        int idmabn = 0;
        class datasoureBN
        {
        ...
        List<datasoureBN> _ldsBN = new List<datasoureBN>();
        ...
        private void Frm_XuatNgoaiTru_Load(object sender, EventArgs e)
        {
        ...
        private void btnXuatDuoc_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txtMaBNhan.Text))
            {
                ...
                var checkton = _medicinesProvider.CheckTonXuatDuoc("xuatnt", txtIDDon.Text);
                if (string.IsNullOrEmpty(checkton) || checkton != "0")
                {
                    ...
                    else
                    {
                        ...
                        if (_dataContext.SaveChanges() >= 0)
                        {
                            ...
                            foreach (var i in _lDThuocct)
                            {
                                ...
                                #region Đổ tồn bảng MedicinesList
                                int ppxuat = _medicinesProvider.GetPPXuat(makho);

                                if (ppxuat == 1)
                                {
                                    _medicinesProvider.UpdateMedicineListPPX1((int)i.MaDV, i.DonGia, (int)i.MaKP, (int)i.MaKXuat, i.SoLuong, 1);
                                }
                                else if (ppxuat == 3)
                                {
                                    _medicinesProvider.UpdateMedicineListPPX3((int)i.MaDV, i.DonGia, i.SoLo, (DateTime)i.HanDung, (int)i.MaKP, (int)i.MaKXuat, i.SoLuong, 1);
                                }
                                #endregion
        ...
        private bool XoaXuat(int _id, int _mbn)
        {
            try
            {
                var xoac = _dataContext.NhapDs.FirstOrDefault(p=> p.SoPL == _id);
                ...
                {
                    var _xoa = _dataContext.NhapDcts.FirstOrDefault(p => p.IDNhapct == xoa.IDNhapct);
                    _dataContext.NhapDcts.Remove(_xoa);

                    #region Đổ tồn bảng MedicinesList
                    int ppxuat = _medicinesProvider.GetPPXuat((int)xoac.MaKP);

                    if (ppxuat == 1)
                    {
                        _medicinesProvider.UpdateMedicineListPPX1((int)_xoa.MaDV, _xoa.DonGia, (int)xoac.MaKP, (int)xoac.MaKPnx, -_xoa.SoLuongX, 1);
                    }
                    else if (ppxuat == 3)
                    {
                        _medicinesProvider.UpdateMedicineListPPX3((int)_xoa.MaDV, _xoa.DonGia, _xoa.SoLo, (DateTime)_xoa.HanDung, (int)xoac.MaKPnx, (int)xoac.MaKP, -_xoa.SoLuongX, 1);
                    }
                    #endregion
                ...
                var dthuoc = _dataContext.DThuocs.FirstOrDefault(p => p.IDDon == iddon && p.Status != -1);
                if (dthuoc != null)
                    dthuoc.Status = 0;
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            ...
            frmIn frm = new frmIn();
            BaoCao.rep_PhieuPhatThuoc rep = Frm_XuatNgoaiTru.innhieudon(_dataContext, id, minid, makpx);
            foreach (var item in x1.Where(p => p.IDDon != minid))
            {
                BaoCao.rep_PhieuPhatThuoc rep2 = Frm_XuatNgoaiTru.innhieudon(_dataContext, id, item.IDDon, item.MaKXuat ?? 0);
