...
namespace QLBV.DungChung
{
    ...
    {
        GridLookUpEdit _currentGridLookUpEdit;
        ...
        public GridCheckMarksSelection(GridLookUpEdit gridLookUpEdit)
            : this()
        {
            CurrentRepository = gridLookUpEdit;
        }

        public GridLookUpEdit CurrentRepository
        {
            get { return _currentGridLookUpEdit; }
            set
            {
                if (_currentGridLookUpEdit != value)
                {
                    Detach();
                    Attach(value);
                }
        ...
        protected virtual void Attach(GridLookUpEdit rep)
        {
            if (rep == null) return;
            selection.Clear();
            _currentGridLookUpEdit = rep;

            edit = _currentGridLookUpEdit.Properties.View.GridControl.RepositoryItems.Add("CheckEdit") as RepositoryItemCheckEdit;

            GridColumn column = _currentGridLookUpEdit.Properties.View.Columns.Add();
            ...
            _currentGridLookUpEdit.Properties.View.Click += new EventHandler(View_Click);
            _currentGridLookUpEdit.Properties.View.CustomDrawColumnHeader += new ColumnHeaderCustomDrawEventHandler(View_CustomDrawColumnHeader);
            _currentGridLookUpEdit.Properties.View.CustomDrawGroupRow += new RowObjectCustomDrawEventHandler(View_CustomDrawGroupRow);
            _currentGridLookUpEdit.Properties.View.CustomUnboundColumnData += new CustomColumnDataEventHandler(view_CustomUnboundColumnData);
            _currentGridLookUpEdit.Properties.View.KeyDown += new KeyEventHandler(view_KeyDown);
        }

        protected virtual void Detach()
        {
            if (_currentGridLookUpEdit == null) return;
            if (edit != null)
            {
                _currentGridLookUpEdit.Properties.View.GridControl.RepositoryItems.Remove(edit);
                edit.Dispose();
            }
            _currentGridLookUpEdit.Properties.View.Click -= new EventHandler(View_Click);
            _currentGridLookUpEdit.Properties.View.CustomDrawColumnHeader -= new ColumnHeaderCustomDrawEventHandler(View_CustomDrawColumnHeader);
