...
{
    public partial class rep_PhieuXN_VKLao : DevExpress.XtraReports.UI.XtraReport
    {
        public rep_PhieuXN_VKLao()
        {
            InitializeComponent();
        }
        internal void dataBindDing(int idCLS)
        {
            ...
            if (bn != null)
            {
                ...
                celTenBNhan.Text = (DungChung.Bien.MaBV == "01071" || DungChung.Bien.MaBV == "01049" || DungChung.Bien.MaBV == "12345" || DungChung.Bien.MaBV == "24297" || DungChung.Bien.MaBV == "56789") ? bn.TenBNhan.ToUpper() : bn.TenBNhan;
                if (bn.Tuoi != null)
                ...
                _maBN = bn.MaBNhan;
                if (!String.IsNullOrEmpty(bn.MaTinh))
                {
                    var tinh = data.DmTinhs.Where(p => p.MaTinh == bn.MaTinh).Select(p => p.TenTinh).FirstOrDefault();
                    celTinh.Text = tinh ?? "";
                }

                if (!String.IsNullOrEmpty(bn.MaHuyen))
                {
                    var huyen = data.DmHuyens.Where(p => p.MaHuyen == bn.MaHuyen).Select(p => p.TenHuyen).FirstOrDefault();
                    celQuanHuyen.Text = huyen ?? "";
                }

                if (!String.IsNullOrEmpty(bn.MaXa))
                {
                    var xa = data.DmXas.Where(p => p.MaXa == bn.MaXa).Select(p => p.TenXa).FirstOrDefault();
                    celPhuongXa.Text = xa ?? "";
                }
                ...
                if (bn.thangTheoDoi != null && bn.thangTheoDoi > 0)
                {
                    celTheoDoi.Text = "x";
                    ...
                    else if(bn.TienSuDTri != null && bn.TienSuDTri ==0)
                ...
                if (qcd != null)
                {
                    ...
                    {
                        celYC1.Text = "x";
                        if (mau != null && DungChung.Bien.MaBV != "12122")
                ...
                // ngày thực hiện:
                if (bn.NgayKQ != null)
                    celNgayTH.Text = bn.NgayKQ.Value.Hour.ToString("D2") + " giờ " + bn.NgayKQ.Value.Minute.ToString("D2") + " ngày " + bn.NgayKQ.Value.ToString("dd/MM/yyyy");
                //Loại bệnh phẩm
                if (!String.IsNullOrEmpty(bn.BenhPham))
                {
                    ...
                    {
                        celBPhanKhac.Text = "x";
                        celDSBPhanKhac.Text = bn.BenhPham;
                    }
                }
                #endregion
                ...
                var qclsct = (from cls in data.CLScts.Where(p => p.IDCD == qcd.cd.IDCD)
                              ...
                              select new { cls.MaDVct, cls.KetQua, cls.STTHT, dvct.STT }).OrderBy(p => p.STTHT).ToList();
                if (qcd != null)
                {
                    //nhuộm soi
