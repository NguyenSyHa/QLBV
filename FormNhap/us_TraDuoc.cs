...
{
    public partial class us_TraDuoc : DevExpress.XtraEditors.XtraUserControl
    {
        public us_TraDuoc()
        {
            InitializeComponent();
        ...
        private bool KT()
        {
            ...
            {
                MessageBox.Show("Bạn chưa chon kho nhận!");
                lupMaKP.Focus();
                return false;
            }
            if (string.IsNullOrEmpty(lupMaXP.Text))
            {
                MessageBox.Show("Bạn chưa chon nơi trả!");
                lupMaKP.Focus();
                return false;
            }
        ...
        private void TimKiem()
        {
            ...
            _lNhapDuoc = (from nd in _data.NhapDs.Where(p => p.PLoai == 1 && p.KieuDon == 2)
                          where (nd.NgayNhap >= _dttu && nd.NgayNhap <= _dtden)
                          where (nd.SoCT.Contains(_soPhieu))
                          where (nd.MaKP == (_makho))
                          where (nd.MaKPnx == (_mkpnx) || _mkpnx == 0)
        ...
        private void usXuatDuoc_Load(object sender, EventArgs e)
        {
            ...
            var qi = (from KhoaKham in _data.KPhongs.Where(p => p.Status == 1 && p.PLoai != null)
                      where (KhoaKham.PLoai.Contains("Lâm sàng") || KhoaKham.PLoai.Contains("khu vực") || KhoaKham.PLoai.ToUpper().Contains("XÃ PHƯỜNG") || KhoaKham.PLoai.Contains("Khoa dược"))
            ...
            //           group new {dv,nduocct} by new {dv.MaDV,dv.TenDV,dv.DonVi} into kq
            //           select new { kq.Key.MaDV, kq.Key.TenDV, kq.Key.DonVi,soluongton=kq.Sum(p=>p.nduocct.SoLuongN)-kq.Sum(p=>p.nduocct.SoLuongX) }).OrderBy(p => p.TenDV).ToList();// join voi nhap duoc
            //if (sql.Count > 0)
            //{
            //    lupMaDuoc.DataSource = sql.ToList();
            //}
            lupMaXP.Properties.DataSource = qi.Where(p => p.PLoai.ToUpper().Contains("XÃ PHƯỜNG")).ToList();
        ...
        private void grvNhapCT_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            switch (e.Column.Name)
            {
                case "colMaDV":
                    ...
                    grvNhapCT.SetRowCellValue(e.RowHandle, colDonGia, 0);
                ...
                case "colSoLuong":
                    if (grvNhapCT.GetFocusedRowCellValue(colSoLuong) != null && grvNhapCT.GetFocusedRowCellValue(colSoLuong).ToString() != "")
                    {
                        ...
                        if (a >= 0)
                        {
                            string _madv = grvNhapCT.GetFocusedRowCellValue(colMaDV).ToString();
                            for (int i = 0; i <= grvNhapCT.RowCount; i++)
                            {
                                if (grvNhapCT.GetRowCellValue(i, colMaDV) != null && grvNhapCT.GetRowCellValue(i, colDonGia) != null)
                                    ...
                                    {
                                        if (_madv == grvNhapCT.GetRowCellValue(i, colMaDV).ToString() && _dongia == Convert.ToDouble(grvNhapCT.GetRowCellValue(i, colDonGia)))
                                        {
                            ...
                            switch (TTLuu)
                            {
                                case 1: // khi tao don moi
                                    ...
                                    if (ton < 0)
                                        ton = 0;
                                    ton = 100000; // không tính tồn
                                    if (a <= ton || !chk_HT.Checked)//DungChung.Bien.SoLuongTon)
                                    {
                                        if (grvNhapCT.GetFocusedRowCellValue(colDonGia) != null && grvNhapCT.GetFocusedRowCellValue(colDonGia).ToString() != "")
                                        {
                                            double b = Convert.ToDouble(grvNhapCT.GetFocusedRowCellValue(colDonGia));
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            if (_sua)
            {
                ...
                txtSoCT.Focus();
                TTLuu = 2;
                lupMaXP_EditValueChanged(sender, e);
            }
        }
        ...
        private void grvNhapCT_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            ...
            {
                if (grvNhapCT.IsNewItemRow(e.RowHandle))
                {
                    grvNhapCT.DeleteRow(e.RowHandle);
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            // //FormThamSo.frmTsBcSuDungThuoc frm106 = new FormThamSo.frmTsBcSuDungThuoc(id);
            //cavithao
            //FormThamSo.frmTsBcSuDungThuoc frmu106 = new FormThamSo.frmTsBcSuDungThuoc();
            //frm106.ShowDialog();
        ...
        private void lupMaKP_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (lupMaKP.EditValue != null)
                makho = Convert.ToInt32(lupMaKP.EditValue);
            if ((TTLuu == 1 || TTLuu == 2) && chk_HT.Checked)
            {
                var duoc = (from tenduoc in _data.DichVus.Where(p => p.PLoai == 1)
                            ...
                            where (nduoc.PLoai == 2 && nduoc.MaKPnx == (makpx))
        ...
        private void chk_HT_CheckedChanged(object sender, EventArgs e)
        {
            lupMaKP_EditValueChanged(sender, e);
        }

        private void chk_HT_CheckStateChanged(object sender, EventArgs e)
        {

        }

        private void chk_HT_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            if (TTLuu == 2 || TTLuu == 1)
            {
                if (grvNhapCT.RowCount > 0)
                {
                    if (grvNhapCT.RowCount == 1)
                    {
                        if (grvNhapCT.GetRowCellValue(1, colMaDV) != null && grvNhapCT.GetRowCellValue(1, colMaDV).ToString() != "")
                        {
                            MessageBox.Show("Chứng từ đã có thuốc, bạn không thể thay đổi");
                            e.Cancel = true;
                        }
                    }
                    else
                    {
                        MessageBox.Show("Chứng từ  có thuốc, bạn không thể thay đổi");
                        e.Cancel = true;
                    }
                }
            }
