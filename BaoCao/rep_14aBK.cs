...
{
    public partial class rep_14aBK : DevExpress.XtraReports.UI.XtraReport
    {
        public rep_14aBK()
        {
            InitializeComponent();
        ...
        public void BindingData()
        {
            ...
            colGFDungtuyen.DataBindings.Add("Text", DataSource, "Soluotdungtuyen");
        ...
        private void soluot(string _HangBV)
        {
            ...
            int thang = Convert.ToInt32(ngaytu.Value);
            string ten = ngayden.Value.ToString();
            int TD = Convert.ToInt32(_HangBV);
            var q = (from bn in Data.BenhNhans.Where(p=>p.TenBNhan.Contains(ten))
                     //join BV in Data.BenhViens.Where(p => p.TuyenBV.Trim()== (_HangBV)) on bn.MaCS equals BV.MaBV
                     join nhom in Data.DTuongs on bn.SThe equals nhom.MaDTuong
                     join vp in Data.VienPhis on bn.MaBNhan equals vp.MaBNhan
                     where (vp.NgayTT.Value.Month==thang)
                     where (nhom.Nhom == "Nhóm 1" || nhom.Nhom == "Nhóm 2" || nhom.Nhom == "Nhóm 3" || nhom.Nhom == "Nhóm 4" || nhom.Nhom == "Nhóm 5" || nhom.Nhom == "Nhóm 6")
                     group new { bn, nhom } by new { bn.MaBNhan, bn.Tuyen, nhom.Nhom } into kq
                     select new { tuyen = kq.Key.Tuyen, SL = kq.Key.MaBNhan, NH = kq.Key.Nhom }).ToList();
            if (q.Count > 0)
            {
