...
{
    public partial class rep_ChamCongPhauThuat_01830 : DevExpress.XtraReports.UI.XtraReport
    {
        public rep_ChamCongPhauThuat_01830()
        {
            InitializeComponent();
        }
                ...
                for (int i = 0; i < arrDSCB.Count(); i++)
                {
                    switch (i)
                    {
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            tenCB = arrDSCB[4].Split(' ');
                            if (tenCB.Count() >= 4)
                            {
                                colPTVPhu2.Text = tenCB[tenCB.Count() - 2] + " " + tenCB[tenCB.Count() - 1];
                            }
                            else
                            {
                                colPTVPhu2.Text = tenCB[tenCB.Count() - 1];
                            }
                            break;
                        case 5:
                            tenCB = arrDSCB[5].Split(' ');
                            if (tenCB.Count() >= 4)
                            {
                                colPTVPhu3.Text = tenCB[tenCB.Count() - 2] + " " + tenCB[tenCB.Count() - 1];
                            }
                            else
                            {
                                colPTVPhu3.Text = tenCB[tenCB.Count() - 1];
                            }
                            break;
                        default:
                            break;
